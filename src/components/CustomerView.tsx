...
import { ShoppingBag, Search, ChevronRight, Check, MapPin, Phone, AlertCircle, RefreshCw, X, ArrowLeft, ArrowRight, CreditCard, Plus } from 'lucide-react';
...
export default function CustomerView({ settings, menu, onOrderPlaced }: CustomerViewProps) {
  ...
  const updateAddCartQty = (id: string, delta: number) => {
    setAddCart(prev => {
      const current = prev[id] || 0;
      const next = current + delta;
      if (next <= 0) {
  ...
  const getAddCartTotal = () => {
    return Object.entries(addCart).reduce((sum, [id, qty]) => {
      const item = menu.find(m => m.id === id);
      return sum + (item ? item.price * (qty as number) : 0);
    }, 0);
  };
  ...
  const handleAddItemsSubmit = async () => {
    ...
    setError('');
    const items: OrderItem[] = Object.entries(addCart).map(([id, qty]) => {
      const item = menu.find(m => m.id === id)!;
      return {
        menuId: item.id,
        name: item.name,
        price: item.price,
        quantity: qty as number,
        notes: addCartNotes[id] || undefined
      };
    });
    try {
        ...
        <div className="max-w-md mx-auto px-4 mt-6">
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
            {[
              ...
              { id: 'cemilan', label: 'Cemilan & Pastry' }
            ].map(cat => (
              <button
                key={cat.id}
              ...
              {filteredMenu.map(item => (
                ...
                >
                  <div className="relative">
                    ...
                    <span className={`absolute top-2 left-2 text-[9px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded-md ${item.category === 'makanan' ? 'bg-red-500/80 text-white' :
                      item.category === 'minuman' ? 'bg-blue-500/80 text-white' :
                  ...
                  <div className="p-2.5 space-y-1.5">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="font-extrabold text-brand-forest text-sm">{formatPrice(item.price)}</span>
                      {item.isAvailable && (
                        cart[item.id] ? (
                          <div className="flex items-center bg-brand-badge-bg rounded-lg p-0.5 border border-brand-badge-border">
                            <button
                              onClick={() => updateCartQuantity(item.id, -1)}
                              className="w-6 h-6 flex items-center justify-center font-bold text-brand-forest hover:bg-brand-badge-bg rounded-md active:scale-90 transition-all text-xs"
                            >
                              -
                            </button>
                            <span className="w-5 text-center text-[10px] font-bold text-brand-deep">{cart[item.id]}</span>
                            <button
                              onClick={() => updateCartQuantity(item.id, 1)}
                              className="w-6 h-6 flex items-center justify-center font-bold text-brand-forest hover:bg-brand-badge-bg rounded-md active:scale-90 transition-all text-xs"
                            >
                          ...
                          <button
                            onClick={() => updateCartQuantity(item.id, 1)}
                            className="bg-brand-forest hover:bg-brand-deep active:scale-90 text-white text-[10px] font-bold px-2.5 py-1.5 rounded-lg shadow-sm cursor-pointer transition-all"
                          >
                            + Tambah
                          </button>
                        )
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="bg-brand-forest p-2 rounded-xl relative">
                    <ShoppingBag size={20} />
                    <span className="absolute -top-1.5 -right-1.5 bg-yellow-500 text-yellow-950 text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center ring-2 ring-brand-forest">
                      {getCartItemCount()}
                    </span>
                  </div>
                      ...
                      <div key={id} className="space-y-1.5">
                        <div className="flex justify-between items-center gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="text-brand-forest font-semibold text-xs">{formatPrice(item.price)}</span>
                          </div>
                          <div className="flex items-center gap-3">
                  ...
                  {menu.filter(m => m.isAvailable).map(item => (
                    <div key={item.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0 ml-3">
                        <button
                          onClick={() => updateAddCartQty(item.id, -1)}
                        ...
                        <span className="w-8 text-center font-bold text-sm text-slate-800">{addCart[item.id] || 0}</span>
                        <button
                          onClick={() => updateAddCartQty(item.id, 1)}
